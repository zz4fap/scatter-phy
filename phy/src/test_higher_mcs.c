...
void parse_args(int argc, char **argv, app_args_t *app_args) {
  ...
  while((opt = getopt(argc, argv, "bcmstvw")) != -1) {
    switch(opt) {
    case 'b':
...
int main(int argc, char **argv) {
  ...
  // ***************************** Initialization ******************************
  double SNR_VECTOR[NOF_SNR_VALUES] = {27.0, 26.0, 25.0, 24.0, 23.0, 22.0, 21.0, 20.0, 19.0, 18.0, 17.0, 16.0, 15.0, 14.0, 13.0, 12.0, 11.0, 10.0, 9.0, 8.0, 7.0, 6.0, 5.0, 4.0, 3.0, 2.0, 1.0, 0.0, -1.0, -2.0, -3.0};
  ...
  if(srslte_sync_init(&sfind, frame_len, frame_len, srslte_symbol_sz(cell.nof_prb), 0, app_args.initial_subframe_index)) {
  ...
  for(uint32_t n_idx = 0; n_idx < NOF_SNR_VALUES; n_idx++) {
    ...
    for(uint32_t iter = 0; iter < NOF_ITERATIONS; iter++) {
      ...
      if(app_args.transmit_pdsch) {
        ...
        if(srslte_pdsch_cfg_scatter(&pdsch_cfg, app_args.sch_temporal_diversity, SRSLTE_PSS_LEN, cell, &grant, DEFAULT_CFI, app_args.subframe_number, 0)) {
