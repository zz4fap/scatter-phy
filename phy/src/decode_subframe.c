...
int main(int argc, char **argv) {
  ...
  srslte_sync_set_N_id_2(&sfind, 0);
    ...
    srslte_sync_find_new(&sfind, subframe_buffer, -offset, &peak_position);
...
//    printf("----> PSS-based CFO estimation: %1.4f\n", mean_cfo2*15000.0);
    ...
    float cfo_est = srslte_sync_cfo_estimate_cp(&sfind, subframe_buffer);
//    printf("----> CP-based CFO estimation: %1.4f\n", cfo_est*15000.0);
    ...
    pdsch_num_rxd_bits = srslte_ue_dl_decode_new(&ue_dl, subframe_buffer, decoded_data, SUBFRAME_NUMBER, dci_msg);
