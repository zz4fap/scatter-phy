...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
	...
	size_t channelizer_rate = McFsource_rate / vChannels;		// Channelizer data output rate
	 ...
	 ***********************************************************************/
	uhd::rfnoc::graph::sptr rx_graph = usrp->create_graph("rfnoc_McFsource_to_file");
	usrp->clear();

	// Flowgraph  McFsource -> channelizer -> File
	std::cout << "Connecting " << McFsource_ctrl_id << " ==> " << channelizer_ctrl_id << std::endl;
	rx_graph->connect(McFsource_ctrl_id, uhd::rfnoc::ANY_PORT, channelizer_ctrl_id, uhd::rfnoc::ANY_PORT);
	...
	streamer_args["block_id"] = channelizer_ctrl_id.to_string();
...
#define recv_to_file_args() \
	(rx_stream, file, spb, channelizer_rate, total_num_samps, total_time, bw_summary, stats, enable_size_map, continue_on_bad_packet)
	//recv to file
