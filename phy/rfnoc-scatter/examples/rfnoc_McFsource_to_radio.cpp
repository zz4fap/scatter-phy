...
int UHD_SAFE_MAIN(int argc, char *argv[])
{
	 ...
	 * Set up DUC
	 ***********************************************************************/
	size_t McFsource_out_rate = 1e3  * sample_len_1ms;			// McFsource output data rate

	duc_ctrl->set_args(uhd::device_addr_t(str(boost::format("output_rate=%lf,input_rate=%lf") % rate % McFsource_out_rate)));
	...
	// Flowgraph  McFsource -> DUC -> Radio
	std::cout << "Connecting " << McFsource_ctrl_id << " ==> " << duc_ctrl_id << std::endl;
	McF_graph->connect(McFsource_ctrl_id, uhd::rfnoc::ANY_PORT, duc_ctrl_id, uhd::rfnoc::ANY_PORT);
