...
def sendRxCtrlToPhy(lc, bandwidth, chan, mcs_idx, rx_gain, num_data_slots, seq_number, send_to_id, phy_id, module, timestamp, debug):
    ...
    internal.receive.basic_ctrl.timestamp    = timestamp
...
def sendTxCtrlToPhy(lc, chan, bandwidth, mcs_idx, gain_value, slots, sequence_number, send_to_id, module, data, rfboost, length, phy_id, timestamp, debug):
            ...
            try:
                ...
                if(internal.sendr.result > 0 and internal.sendr.result == PHY_SUCCESS):
                    ...
                    if(internal.transaction_index == 66):
                       ...
                       if(saveStatsToFile == True):
                          f.write("%d\t%1.5f\t%d\t%1.4f\t%d\t%.10f\t%1.4f\t%d\t%d\t%d\n" % (internal.receiver.result, internal.receiver.stat.rx_stat.cfo, internal.receiver.stat.mcs, internal.receiver.stat.rx_stat.rssi, internal.receiver.stat.rx_stat.cqi, internal.receiver.stat.rx_stat.noise, internal.receiver.stat.rx_stat.sinr, total_pkt_number, in_seq_cnt, out_of_seq_cnt))
...
if __name__ == '__main__':
    ...
    sendRxCtrlToPhy(lc, bandwidth, rx_channel, mcs_idx, rx_gain, num_of_rx_slots, 66, send_to, phy_id, source_module, timestamp, debug)

    if(phy_id == 0):
        other_phy_id = 1
    else:
        other_phy_id = 0
    sendRxCtrlToPhy(lc, bandwidth, rx_channel+1, mcs_idx, rx_gain, num_of_rx_slots, 66, send_to, other_phy_id, source_module, timestamp, debug)
    ...
    while(True):
        ...
        sendTxCtrlToPhy(lc, tx_channel, bandwidth, mcs_idx, tx_gain, num_of_tx_slots, seq_number, send_to, source_module, data, rfboost, length, phy_id, timestamp, debug)

        # PHY ID 1.
        # Generate data according to sequence number.
        if(0):
            data1, length1 = generateData3(20, seq_number-99, bandwidth, mcs_idx)
            sendTxCtrlToPhy(lc, 1, bandwidth, mcs_idx, 35, 20, seq_number, send_to, source_module, data1, rfboost, length1, phy_id+1, debug)
