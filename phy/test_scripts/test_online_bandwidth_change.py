...
def inputOptions(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hdpstc:b:",["help","debug","profile","single","txstats","cid","bw=","txchannel=","rxchannel=","mcs=","txgain=","rxgain="])
    ...
    return debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, txchannel, rxchannel, mcs, tx_gain, rx_gain
...
def sendTxCtrlToPhy(lc, chan, bandwidth_index, mcs_idx, gain_value, slots, sequence_number, send_to_id, module, data, debug):
    ...
    mcs         = mcs_idx           # It has no meaning for RX. MCS is recognized automatically by receiver. MCS varies from 0 to 28.
    txchannel   = chan              # By default use channel 0.
    slot        = 0                 # Slot number (not used now, for future use)
    ...
    internal.send.basic_ctrl.ch           = txchannel
...
if __name__ == '__main__':
    ...
    debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, txchannel, rxchannel, mcs, tx_gain, rx_gain = inputOptions(sys.argv[1:])
    ...
    print("PRB:", nof_prb, "- BW index:", bandwidth, "- MCS:", mcs, "- TB size:",getTransportBlockSize(bandwidth, mcs))
    ...
    sendRxCtrlToPhy(lc, bandwidth, rxchannel, mcs, rx_gain, num_of_rx_slots, 66, source_module, debug)
    ...
    while(True):
        ...
        sendTxCtrlToPhy(lc, txchannel, bandwidth, mcs, tx_gain, num_of_tx_slots, seq_number, send_to, source_module, data, debug)
