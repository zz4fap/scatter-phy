...
def inputOptions(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hdpswi:b:m:t:r:",["help","debug","profile","single","waittxstats","sendtoid=","bw","mcs=","txgain=","rxgain=","txslots=","rxslots=","txchannel=","rxchannel=","stats","rfboost=","numpktstotx=", "filename="])
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            print("total pkt to send ", num_of_packets_to_tx)
        elif opt in ("--filename"):
            filename = str(arg)
            print("filename of the log ", filename)
    

    return debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs, txgain, rxgain, tx_slots, rx_slots, tx_channel, rx_channel, debugStats, rfboost, num_of_packets_to_tx, filename
...
def receiveStatisticsFromPhyThread(lc, stats_queue, num_of_tx_slots, wait_for_tx_stats, debug, debugStats, filename):
                ...
                if(internal.sendr.result > 0 and internal.sendr.result == PHY_SUCCESS):
                    ...
                    if(internal.transaction_index == 66):
                       ...
                       if(debugStats == True):
                           ...
                           avg_sinr = avg_sinr + internal.receiver.stat.rx_stat.sinr
                           arr_cqi.append(internal.receiver.stat.rx_stat.cqi)
                           arr_rssi.append(internal.receiver.stat.rx_stat.rssi)
                           arr_noise.append(internal.receiver.stat.rx_stat.noise)
                           arr_sinr.append(internal.receiver.stat.rx_stat.sinr)
                           if(total_pkt_number%1000 == 0):
                                ...
                                print("Avg. SINR: %f" % (avg_sinr/avg_counter))
    ...
    if(getExitFlag() == True or quit_flag == True):
        

        # ttt = [1.3, 3, -65, 78]
        # with open("/logs/test.csv","w") as output:
            # writer = csv.writer(output)
            # writer.write(map(lambda y: [y], ttt))
        f = open(filename, 'w')
        f.write("rssi: \n")
        for item in arr_rssi:
            f.write ("%.2f," % item)
        f.write("\ncqi: \n")
        for item in arr_cqi:
            f.write ("%s," % item)  
        ...
        f.write("\nnoise_scale: %s\n" % noise_scale)
        f.write("\noise: \n")
        for item in arr_noise:
            item = item * noise_scale
            f.write ("%.4f," % item)     
           
        f.write("\ntotal_rx_pkts: %s" % total_pkt_number)
        f.write("\nsinr: \n")
        for item in arr_sinr:
            f.write ("%s," % item)  
        f.close()
        print("Finish statistics thread, in total received ", total_pkt_number, "packets")  
        os._exit(0)

quit_flag = False
# a global var, temp solution for now 
if __name__ == '__main__':

    # Parse any input option.
    debug, profiling, single, wait_for_tx_stats, send_to_id, nof_prb, mcs, txgain, rxgain, tx_slots, rx_slots, tx_channel, rx_channel, debugStats, rfboost, num_of_packets_to_tx, filename = inputOptions(sys.argv[1:])
    ...
    try:
        thread_id = _thread.start_new_thread( receiveStatisticsFromPhyThread, (lc, stats_queue, num_of_tx_slots, wait_for_tx_stats, debug, debugStats, filename, ) )
    except:
