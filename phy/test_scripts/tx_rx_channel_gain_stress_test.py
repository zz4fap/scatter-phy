...
def stressRxParametersThread(lc, bandwidth, rx_channel, mcs_idx, rx_gain, num_of_rx_slots, seq_num, send_to, phy_id, source_module, debug):
    ...
    while(getExitFlag() == False):
        ...
        print("[Rx parameters] Rx channel: %d - Rx gain: %d - PHY ID: %d" % (rx_channel, rx_gain, phy_id))
        ...
        # Wait sometime before changing parameters again.
        time.sleep(0.002)
...
if __name__ == '__main__':
    ...
    try:
        thread_id = _thread.start_new_thread( stressRxParametersThread, (lc, bandwidth, rx_channel, mcs_idx, rx_gain, num_of_rx_slots, seq_number, send_to, phy_id, source_module, debug, ) )
    except:
    ...
    while(True):
        ...
        # Change Tx parameters randomly.
        tx_channel = random.randint(0, 10);
        tx_gain = random.randint(0, 30);
        phy_id = random.randint(0, 1);

        print("[Tx parameters] Tx channel: %d - Tx gain: %d - PHY ID: %d" % (tx_channel, tx_gain, phy_id))
