...
BATCH_ID = [20819 20820 20821 20822 20823 20841];

for id_cnt=1:1:length(BATCH_ID)
    
    folder = sprintf('/home/zz4fap/Downloads/link_level_check/LBT/OLD_14/RESERVATION-%d-all-logs/',BATCH_ID(id_cnt));
    ...
    string4_to_look_for = ' - [RX STATS]: Detection errors:';
                ...
                while ischar(tline)
                    ...
                    if(~isempty(ret))
                        ...
                        if(isempty(wrong_rx_stats_struct))
                            ...
                            wrong_rx_stats.dci = [];
                            wrong_rx_stats.noi = [];
                            wrong_rx_stats.decoding_errors = [];
                        ...
                        if(length(desired_str_delimiters) == 14)
                            substring = tline(desired_str_delimiters(14):end);
                            end_of_desired_str = find(substring=='D',1);
                            if(length(end_of_desired_str) == 1)
                                noise_string = substring(3:end_of_desired_str(1)-4);
                                noise = str2num(noise_string);
                        ...
                        % Decoding errors
                        desired_str_delimiters = find(tline==':',15);
                        if(length(desired_str_delimiters) == 15)
                            decoding_errors_str = tline(desired_str_delimiters(15)+2:end);
                            decoding_errors = str2num(['int64(' decoding_errors_str ')']);
                            wrong_rx_stats.decoding_errors = [wrong_rx_stats.decoding_errors, decoding_errors];
                            %fprintf(1,'\t Noise Value: %f\n',noise);
                        else
                            wrong_rx_stats.decoding_errors = [wrong_rx_stats.decoding_errors, wrong_rx_stats.decoding_errors(length(wrong_rx_stats.decoding_errors))];
                        end                                               
