...
fir_coef = 1; %fir1(512, (64*delta_f)/(NDFT*delta_f)).';
...
for snrIdx = 1:1:length(SNR)
    ...
    for iter=1:1:numIter
        ...
        filt_rx_signal = conv(rx_signal, fir_coef);
        ...
        %% ------------------------------------------------------------------------
        % Coherent PSS-based CFO estimation.

        RX_PSS = fft(rx_signal, NDFT);
        %figure;plot(abs(RX_PSS));
        
        % Estimate channel.
        CE = RX_PSS./pss_zero_pad;
        
        
        y = RX_PSS.*conj(pss_zero_pad);
        
        ycorr0 = sum(y(1:1:NDFT/2));
            
        ycorr1 = sum(y((NDFT/2)+1:1:NDFT));
        
        cfo_est_coherent = angle(conj(ycorr0).*ycorr1)/pi;
            
        cfo_freq_est_coherent = cfo_est_coherent*15000;        
        
        error_coherent = abs((cfo_freq - cfo_freq_est_coherent)).^2;
        
        a=1;
        
        if(0)
            RX_PSS_new = RX_PSS./CE;
            %figure;plot(abs(RX_PSS_new))
            
            rx_pss_new = ifft(RX_PSS_new,NDFT);
            
            y = rx_pss_new.*conj(local_time_domain_pss);
            ...
            error_coherent_avg(snrIdx) = error_coherent_avg(snrIdx) + error_coherent;
        end
        
        
        %% ------------------------------------------------------------------------
        % Frequency-domain PSS-based CFO estimation.        
        a=1;
    ...
    error_coherent_avg(snrIdx) = error_coherent_avg(snrIdx)/numIter;
...
semilogy(SNR, error_coherent_avg);
hold off;
grid on;
xlabel('MSE')
title('CFO estimation')
legend('Traditional','Filtered','Coherent')
