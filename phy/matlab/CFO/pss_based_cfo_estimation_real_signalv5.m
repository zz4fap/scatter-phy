...
error_cp_avg = zeros(1,length(SNR));
for snrIdx = 1:1:length(SNR)
    ...
    for iter=1:1:numIter
        ...
        cfo_signal = ofdm_subframe.*freq_offset(1:1:FRAME_SIZE);
...
%         filtered_rx_signal = ifft(RX_PSS,NDFT);
        ...
        error_cp_avg(snrIdx) = error_cp_avg(snrIdx) + error_cp;
    ...
    error_cp_avg(snrIdx) = error_cp_avg(snrIdx)/numIter;
    ...
    fprintf(1,'CP-based CFO Estimate Error: %1.2e\n', error_cp_avg(snrIdx));
    fprintf(1,'-----------------------------------------------\n');
...
semilogy(SNR, error_cp_avg);
hold off;
grid on;
xlabel('MSE')
title('CFO estimation')
legend('PSS-based','Filtered PSS-based','CP-based')
