...
for i=0:1:ceil(numDataCarriers/NFFT)-1
    start_idx = i*(NFFT)+1;
    stop_idx = (i+1)*NFFT;
    aux = fft(symbolsIn(start_idx:stop_idx),NFFT);
    scfdma = [scfdma; aux];
end

symbolsInSCFDMA = [zeros(offset,1); scfdma; zeros(numFFT(bw_idx)-offset-numDataCarriers,1)];
ifftOut = ifft(ifftshift(symbolsInSCFDMA));
