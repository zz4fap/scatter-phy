...
acpr_freq_offset = 1e6*(0.005:0.05:0.6);
acpr = comm.ACPR('SampleRate', numFFT(bw_idx)*deltaF, ...
    'MainChannelFrequency', 0, ...
    'MainMeasurementBandwidth', (numDataCarriers*deltaF), ...
    'AdjacentChannelOffset', acpr_freq_offset, ...
    'AdjacentMeasurementBandwidth', (numDataCarriers*deltaF), ...
...
ACPR1 = acpr(txSigOFDM);
ACPR2 = acpr(txSigFOFDM);
ACPR3 = acpr(txSigDFTsOFDM);

plot(acpr_freq_offset/1e6, ACPR1, '*-', acpr_freq_offset/1e6, ACPR2, 'o-', acpr_freq_offset/1e6, ACPR3, 's-')
xlabel('Adjacent Channel Offset (MHz)')
ylabel('ACPR (dB)')
legend('OFDM', 'f-OFDM', 'Proposed', 'location', 'best')
grid
