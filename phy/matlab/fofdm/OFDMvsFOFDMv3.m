...
for numSyms=1:1:numOfOFDMSymbols
    ...
    txSigOFDM = [txSigOFDM; [ifftOut(end-cpLen+1:end); ifftOut]];
end
...
[psd,f] = periodogram(txSigOFDM(numFFT+cpLen:end), rectwin(length(txSigOFDM(numFFT+cpLen:end))), numFFT*2, ...
                      1, 'centered'); 
...
signal = rxSigFiltSync((numFFT+cpLen)*(symCounter-1)+1:(numFFT+cpLen)*symCounter);

% Remove cyclic prefix
rxSymbol = signal(cpLen+1:end);
