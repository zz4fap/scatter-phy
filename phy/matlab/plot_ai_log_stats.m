...
for id_cnt=1:1:length(BATCH_ID)
    ...
    for k=3:1:length(subfolders)
        ...
        if(subfolders(k).isdir == 1)
            ...
            dot_mat_file_name = sprintf('%s%s-ai-stats-structs.mat',folder,subfoldername);
            ...
            save(dot_mat_file_name,'acks_struct')
            ...
            for i=1:1:number_of_acks
                ...
                if(at_least_one_ack_received)                 
                    ...
                    title_str = sprintf('Rxed ACK - %s - MAC: %d',trial_name,mac_stats_struct.mac);
                    xlabel('Time [s]');
                    title(title_str);
                    if(is_there_timeout == true)
                    ...
                    saveas_file_name = sprintf('%s%s_mac_%d.png',folder,trial_name,mac_stats_struct.mac);
