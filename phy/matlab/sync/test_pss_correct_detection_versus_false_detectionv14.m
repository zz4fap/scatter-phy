...
enable_ofdm_with_pss_plus_data1       = false;
...
if(enable_ofdm_with_pss_plus_data5)
    ...
    pss = scatter_pss_zc(cell_id, SCATTER_PSS_LEN);

    PSS_GUARD_BAND = ((SRSLTE_PSS_LEN + PSS_GUARD_BAND) - SCATTER_PSS_LEN);
    
    nof_pss_nulls = PSS_GUARD_BAND/2;
    
    % Create OFDM symbol number 6, the one which carries PSS.
    pss_zero_pad = [0;pss((SCATTER_PSS_LEN/2)+1:end);zeros(NDFT-(SCATTER_PSS_LEN+1),1);pss(1:SCATTER_PSS_LEN/2)];
    local_time_domain_pss = (sqrt(NDFT)/sqrt(SCATTER_PSS_LEN))*ifft(pss_zero_pad, NDFT);
    ...
    for snr_idx = 1:1:length(SNR)
        ...
        for n=1:1:NUM_ITER
            ...
            for symb_idx = 1:1:14
                if(symb_idx == 7)
                    % Generate random QPSK data.
                    data = randi([0 3], USEFUL_SUBCARRIERS-SCATTER_PSS_LEN-PSS_GUARD_BAND, 1);
                    % Apply QPSK modulation.
                    data_symbols = qammod(data, 4, 'UnitAveragePower', true);
                    tx_pss_zero_pad = [0; pss((SCATTER_PSS_LEN/2)+1:end); zeros(nof_pss_nulls,1); data_symbols(1:114,1); zeros(83,1); data_symbols(114+1:end,1); zeros(nof_pss_nulls,1); pss(1:SCATTER_PSS_LEN/2)];
                    tx_time_domain_pss_plus_data = (sqrt(NDFT)/sqrt(SCATTER_PSS_LEN))*ifft(tx_pss_zero_pad, NDFT);
                    tx_time_domain_pss_plus_data_plus_cp = [tx_time_domain_pss_plus_data(end-cpLen_other_symbs(phy_bw_idx)+1:end); tx_time_domain_pss_plus_data];
                    ...
                    tx_data_zero_pad = [0; data_symbols(1:USEFUL_SUBCARRIERS/2,1); zeros(83,1); data_symbols(USEFUL_SUBCARRIERS/2+1:end,1)];
                    tx_time_domain_data = (sqrt(NDFT)/sqrt(SCATTER_PSS_LEN))*ifft(tx_data_zero_pad, NDFT);
...
if(enable_ofdm_with_pss_plus_data5)
    ...
    xlabel('SNR [dB]');
    legend('Correct detection', 'Miss detection', 'False detection');
    titleStr = sprintf('OFDM Symbol with PSS + Data - Averaged-PSR-based Threshold & Longer PSS (%d)', SCATTER_PSS_LEN);
    title(titleStr);
    hold off;
end
