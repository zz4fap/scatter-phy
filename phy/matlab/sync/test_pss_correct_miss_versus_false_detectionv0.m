...
if(enable_ofdm_with_pss_plus_data)
    ...
    for snr_idx = 1:1:length(noise_variance)
        ...
        for n = 1:1:NUM_ITER
            ...
            psr = peak_value/side_lobe_value;
...
if(enable_ofdm_with_pss_plus_data_filter)
    ...
    correct_detection_with_data_filter = zeros(1,length(noise_variance));
    miss_detection_with_data_filter = zeros(1,length(noise_variance));
    ...
    for snr_idx = 1:1:length(noise_variance)
        ...
        for n = 1:1:NUM_ITER
            ...
            if(add_noise == 1)
                noisy_signal = awgn(rx_signal_buffer, noise_variance(snr_idx), 'measured');
            else
            ...
            if(psr >= 3)
                if(peak_pos-(FIR_ORDER/2)-1 == SUBFRAME_LENGTH(phy_bw_idx)/2)
...
if(enable_ofdm_with_pss_plus_data2)
    ...
    for snr_idx = 1:1:length(noise_variance)
        ...
        for n = 1:1:NUM_ITER
            ...
            [side_lobe_value, side_lobe_pos] = max(side_lobe_values_vector);
            
            psr = peak_value/side_lobe_value;
            
            power_conv_avg = sum(power_conv)/length(power_conv);
            
            %threshold = power_conv_avg + power_conv_avg*4; % Good threshold
            %threshold = power_conv_avg + power_conv_avg*0.3;
            
            threshold = min(side_lobe_values_vector)/power_conv_avg;
...
if(enable_ofdm_with_pss_plus_data)
    subplot(2,1,1);
    plot(10*log10(noise_variance), correct_miss_with_data);
    hold on;
    plot(10*log10(noise_variance), false_detection_with_data);
    grid on;
    xlabel('Noise power [dBW]')
    legend('Correct miss', 'False detection');
    title('OFDM Symbol with PSS + Data - PSR-based Threshold');
    hold off;
end

if(enable_ofdm_with_pss_plus_data_filter)
    subplot(2,1,2);
    plot(10*log10(noise_variance), correct_detection_with_data_filter);
    hold on;
    plot(10*log10(noise_variance), miss_detection_with_data_filter);
    grid on;
    xlabel('SNR [dB]')
    legend('Correct miss', 'False detection');
    title('OFDM Symbol with PSS + Data + Filter');
    hold off;
end

if(enable_ofdm_with_pss_plus_data2)
    subplot(2,1,2);
    plot(10*log10(noise_variance), correct_miss_with_data2);
    hold on;
    plot(10*log10(noise_variance), false_detection_with_data2);
    grid on;
    xlabel('Noise power [dBW]')
    legend('Correct miss', 'False detection');
    title('OFDM Symbol with PSS + Data - Noise-based Threshold');
    hold off;
end
