...
if(enable_ofdm_with_pss_plus_data_filter)
    ...
    FILTER_LEN = 62;
    local_filter_pad = [1; ones((FILTER_LEN/2), 1); zeros(NDFT-(FILTER_LEN+1),1); ones((FILTER_LEN/2), 1)];
    local_filter_time_domain = (sqrt(NDFT)/sqrt(FILTER_LEN))*ifft(local_filter_pad, NDFT);
    filter_ = [local_filter_time_domain; zeros(FRAME_SIZE,1)];
    filter_fft = fft(filter_,NDFT2); %ones(NDFT2,1); 
