...
int main(int argc, char **argv) {
  ...
  if (!output_file_name) {
    ...
    if (srate != -1) {
      ...
      float srate_rf = srslte_rf_set_tx_srate(&rf, (double) srate, 0);
      if (srate_rf != srate) {
    ...
    printf("Set TX gain: %.1f dB\n", srslte_rf_set_tx_gain(&rf, rf_gain, 0));
    printf("Set TX freq: %.2f MHz\n",
        srslte_rf_set_tx_freq(&rf, rf_freq, 0) / 1000000);
  }
#endif
  ...
  while ((nf < nof_frames || nof_frames == -1) && !go_exit) {
    ...
    //for (sf_idx = 0; sf_idx < 2 && (nf < nof_frames || nof_frames == -1); sf_idx++) {
      ...
      if (output_file_name) {
        ...
        srslte_rf_send2(&rf, output_buffer, sf_n_samples, true, start_of_burst, end_of_burst, 0);
