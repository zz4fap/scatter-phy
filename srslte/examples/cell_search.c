...
int srslte_rf_recv_wrapper(void *h, void *data, uint32_t nsamples, srslte_timestamp_t *t, size_t channel) {
  ...
  return srslte_rf_recv((srslte_rf_t*) h, data, nsamples, 1, channel);
}
...
double srslte_rf_set_rx_gain_wrapper(void *h, double f) {
  return srslte_rf_set_rx_gain((srslte_rf_t*) h, f, 0);
}
...
int main(int argc, char **argv) {
  ...
  for (freq=0;freq<nof_freqs && !go_exit;freq++) {
    ...
    /* set rf_freq */
    srslte_rf_set_rx_freq(&rf, (double) channels[freq].fd * MHZ, 0);
    ...
    srslte_rf_set_rx_srate(&rf, SRSLTE_CS_SAMP_FREQ, 0);
    INFO("Starting receiver...\n", 0);
    srslte_rf_start_rx_stream(&rf, 0);
    
    n = srslte_ue_cellsearch_scan(&cs, found_cells, NULL, 0); 
    ...
    if (n < 0) {
      ...
      for (int i=0;i<3;i++) {
        if (found_cells[i].psr > 10.0) {
          ...
          int ret = rf_mib_decoder(&rf, &cell_detect_config, &cell, NULL, 0);
          if (ret < 0) {
