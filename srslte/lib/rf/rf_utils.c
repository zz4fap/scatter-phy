...
int rf_rssi_scan(srslte_rf_t *rf, float *freqs, float *rssi, int nof_bands, double fs, int nsamp, size_t channel) {
  int i, j;
  int ret = -1;
  cf_t *buffer;
  double f;
  ...
  srslte_rf_set_rx_gain(rf, 20.0, channel);
  srslte_rf_set_rx_srate(rf, fs, channel);

  for (i=0;i<nof_bands;i++) {
    srslte_rf_stop_rx_stream(rf, channel);

    f = (double) freqs[i];
    srslte_rf_set_rx_freq(rf, f, channel);
    srslte_rf_rx_wait_lo_locked(rf, channel);
    usleep(10000);
    srslte_rf_start_rx_stream(rf, channel);

    /* discard first samples */
    for (j=0;j<2;j++) {
      if (srslte_rf_recv(rf, buffer, nsamp, 1, channel) != nsamp) {
        goto free_and_exit;
      }
    }
...
int srslte_rf_recv_wrapper_cs(void *h, void *data, uint32_t nsamples, srslte_timestamp_t *t, size_t channel) {
 ...
 */
int rf_mib_decoder(srslte_rf_t *rf, cell_search_cfg_t *config, srslte_cell_t *cell, float *cfo, size_t channel) {
  ...
  srslte_rf_set_rx_srate(rf, (float) srate, channel);
  ...
  /* Find and decody MIB */
  ret = srslte_ue_mib_sync_decode(&ue_mib, config->max_frames_pbch, bch_payload, &cell->nof_ports, NULL, channel);
  if (ret < 0) {
...
int rf_cell_search(srslte_rf_t *rf, cell_search_cfg_t *config,
                     int force_N_id_2, srslte_cell_t *cell, float *cfo, size_t channel)
{
  int ret = SRSLTE_ERROR;
  ...
  srslte_rf_set_rx_srate(rf, SRSLTE_CS_SAMP_FREQ, channel);
  ...
  if (force_N_id_2 >= 0) {
    ret = srslte_ue_cellsearch_scan_N_id_2(&cs, force_N_id_2, &found_cells[force_N_id_2], channel);
    max_peak_cell = force_N_id_2;
  } else {
    ret = srslte_ue_cellsearch_scan(&cs, found_cells, &max_peak_cell, channel);
  }
  if (ret < 0) {
 ...
 * -1 on error
 */
int rf_search_and_decode_mib(srslte_rf_t *rf, cell_search_cfg_t *config, int force_N_id_2, srslte_cell_t *cell, float *cfo, size_t channel)
{
  int ret = SRSLTE_ERROR;

  printf("Searching for cell...\n");
  ret = rf_cell_search(rf, config, force_N_id_2, cell, cfo, channel);
  if (ret > 0) {
    printf("Decoding PBCH for cell %d (N_id_2=%d)\n", cell->id, cell->id%3);
    ret = rf_mib_decoder(rf, config, cell, cfo, channel);
    if (ret < 0) {
