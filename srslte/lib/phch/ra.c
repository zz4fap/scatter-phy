...
// Define number of Resource Elements used by SCH in the front of the subframe.
#define NUM_OF_RES_SCH 72

// Obtains a DL grant from a DCI grant for PDSCH.
int srslte_ra_dl_dci_to_grant_scatter(srslte_ra_dl_dci_t *dci, uint32_t nof_prb, uint16_t msg_rnti, srslte_ra_dl_grant_t *grant) {
  bool crc_is_crnti = false;
  if(msg_rnti >= SRSLTE_CRNTI_START && msg_rnti <= SRSLTE_CRNTI_END) {
    crc_is_crnti = true;
  }
  ...
  if(!dl_dci_to_grant_prb_allocation(dci, grant, nof_prb)) {
    // Compute MCS
    if(!dl_dci_to_grant_mcs_scatter(dci, grant, crc_is_crnti)) {
      // Apply Section 7.1.7.3. If RA-RNTI and Format1C rv_idx=0
      if(msg_rnti >= SRSLTE_RARNTI_START && msg_rnti <= SRSLTE_RARNTI_END && dci->dci_is_1c) {
        dci->rv_idx = 0;
      }
    } else {
      return SRSLTE_ERROR;
...
// Loosely based on 7.1.7 in 36.213.
int dl_dci_to_grant_mcs_scatter(srslte_ra_dl_dci_t *dci, srslte_ra_dl_grant_t *grant, bool crc_is_crnti) {
  uint32_t n_prb=0;
  int tbs = -1;
  uint32_t i_tbs = 0;
  ...
  if(!crc_is_crnti) {
    ...
    n_prb = grant->nof_prb;
    grant->mcs.idx = dci->mcs_idx;
    tbs = dl_fill_ra_mcs_scatter(&grant->mcs, n_prb);
    if(tbs) {
      last_dl_tbs[dci->harq_process%8] = tbs;
    } else {
      // For mcs>=29, set last TBS received for this PID
    ...
    if(dci->nof_tb == 2) {
      grant->mcs2.idx = dci->mcs_idx_1;
      tbs = dl_fill_ra_mcs_scatter(&grant->mcs2, n_prb);
    }
  }
  grant->Qm = srslte_mod_bits_x_symbol(grant->mcs.mod);
  if(dci->nof_tb == 2) {
...
int dl_fill_ra_mcs_scatter(srslte_ra_mcs_t *mcs, uint32_t nprb) {
  uint32_t i_tbs = 0;
  int tbs = -1;
  if(mcs->idx < 10) {
    mcs->mod = SRSLTE_MOD_QPSK;
    ...
    mcs->mod = SRSLTE_MOD_64QAM;
    i_tbs = mcs->idx-2;
  } else if(mcs->idx >= 29 && mcs->idx <= 31) {
    mcs->mod = SRSLTE_MOD_64QAM;
    i_tbs = mcs->idx;
  } else {
    fprintf(stderr, "Invalid MCS value.\n");
    return SRSLTE_ERROR;
  }
  if(tbs == -1) {
    tbs = srslte_ra_tbs_from_idx_scatter(i_tbs, nprb);
    if(tbs >= 0) {
      mcs->tbs = tbs;
    }
  }
...
static int get_bw_index(uint32_t n_prb) {
  int ret = -1;
  switch(n_prb) {
    case 6:
    case 7:
...
// Loosely based on Table 7.1.7.2.1-1: Transport block size table on 36.213.
int srslte_ra_tbs_from_idx_scatter(uint32_t tbs_idx, uint32_t n_prb) {
  int ret = -1, bw_index = -1;
  if(n_prb > 0 && n_prb <= SRSLTE_MAX_PRB) {
    if(tbs_idx < 27) {
      ret = tbs_table_scatter[tbs_idx][n_prb - 1];
    } else if(tbs_idx >= 27 && tbs_idx <= 31) {
      bw_index = get_bw_index(n_prb);
      if(bw_index >= 0) {
        ret = 8*tbs_table_scatter_higher_mcs[bw_index][tbs_idx];
      } else {
        ret = SRSLTE_ERROR;
      }
    } else {
  ...
  if(ret == SRSLTE_ERROR) {
    fprintf(stderr, "[RA TBS table] tbs_idx: %d - bw_index: %d - TB size in bits: %d - in bytes: %d\n", tbs_idx, bw_index, ret, ret/8);
  }
  return ret;
}

void srslte_ra_dl_grant_to_nbits_scatter(srslte_ra_dl_grant_t *grant, bool add_sch_to_front, uint32_t eob_pss_len, srslte_cell_t cell, uint32_t sf_idx, srslte_ra_nbits_t *nbits) {
  // Compute number of RE.
  nbits->nof_re   = srslte_ra_dl_grant_nof_re_scatter(grant, add_sch_to_front, eob_pss_len, cell, sf_idx);
  nbits->lstart   = 0; // start from the 0th postion as we don't encode CFI nor DCIs.
  nbits->nof_symb = 2*SRSLTE_CP_NSYMB(cell.cp)-nbits->lstart;
  nbits->nof_bits = nbits->nof_re * grant->Qm;
  //printf("[RA DL] nof_re: %d - nof_bits: %d\n", nbits->nof_re, nbits->nof_bits);
}

// Computes the number of RE for each PRB in the prb_dist structure.
uint32_t srslte_ra_dl_grant_nof_re_scatter(srslte_ra_dl_grant_t *grant, bool add_sch_to_front, uint32_t eob_pss_len, srslte_cell_t cell, uint32_t sf_idx) {

  // Compute total number of RE in a subframe.
  uint32_t nof_re = 0;
  for(uint32_t slot = 0; slot < 2; slot++) {
    for(uint32_t prb_idx = 0; prb_idx < cell.nof_prb; prb_idx++) {
      if(grant->prb_idx[slot][prb_idx]) {
        nof_re += ra_re_x_prb_scatter(sf_idx, slot, prb_idx, cell.nof_prb, cell.nof_ports, cell.cp);
      }
    }
  }

  // Remove REs used by SCH at the front of the first subframe.
  if(add_sch_to_front && sf_idx == 5) {
    nof_re -= NUM_OF_RES_SCH;
  }

  // Remove REs used by EOB PSS signal sequence.
  if(sf_idx == 7) {
    nof_re -= eob_pss_len;
  }
...
// Returns the number of RE in a PRB in a slot and subframe.
uint32_t ra_re_x_prb_scatter(uint32_t subframe, uint32_t slot, uint32_t prb_idx, uint32_t nof_prb, uint32_t nof_ports, srslte_cp_t cp) {

  uint32_t re;

  re = SRSLTE_CP_NSYMB(cp) * SRSLTE_NRE;

  // If it's the prb in the middle, there are less RE due PSS/SSS.
  if(subframe == 5 && (prb_idx >= nof_prb / 2 - 3 && prb_idx < nof_prb / 2 + 3 + (nof_prb%2))) {
    if(slot == 0) {
      re = (SRSLTE_CP_NSYMB(cp) - 2) * SRSLTE_NRE;
    }
...
// Returns the transport block size for scatter PHY.
uint32_t srslte_ra_get_tb_size_scatter(uint32_t prb, uint32_t mcs) {
  return tbs_table_scatter_higher_mcs[prb][mcs];
}
