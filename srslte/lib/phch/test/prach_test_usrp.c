...
int main(int argc, char **argv) {
  ...
  srslte_rf_set_rx_gain(&rf, uhd_rx_gain, 0);
  srslte_rf_set_tx_gain(&rf, uhd_tx_gain, 0);
  srslte_rf_set_rx_freq(&rf, uhd_freq, 0);
  srslte_rf_set_tx_freq(&rf, uhd_freq, 0);
  ...
  srslte_rf_set_tx_srate(&rf, (double) srate, 0);
  sleep(1);
  ...
  while(nframe<nof_frames) {
    ...
    srslte_rf_recv_with_time(&rf, &buffer[flen*nframe], flen, true, &tstamp.full_secs, &tstamp.frac_secs, 0);
    nframe++;
    if (nframe==9 || nframe==8) {
      srslte_timestamp_add(&tstamp, 0, 2e-3-timeadv*1e-6);
      if (nframe==8) {
        srslte_rf_send_timed2(&rf, zeros, flen, tstamp.full_secs, tstamp.frac_secs, true, false, 0);      
        printf("Transmitting zeros\n");        
      } else {
        srslte_rf_send_timed2(&rf, preamble, flen, tstamp.full_secs, tstamp.frac_secs, false, true, 0);      
