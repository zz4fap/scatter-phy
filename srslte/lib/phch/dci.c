...
//******************************************************************************
// Unpacks a DCI message and configures the DL grant object.
int srslte_dci_msg_to_dl_grant_scatter(srslte_dci_msg_t *msg, uint16_t msg_rnti, uint32_t nof_prb, uint32_t nof_ports, srslte_ra_dl_dci_t *dl_dci, srslte_ra_dl_grant_t *grant) {
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  ...
  if(msg != NULL && grant != NULL) {
    ...
    srslte_ra_dl_dci_to_grant_scatter(dl_dci, nof_prb, msg_rnti, grant);
