...
//******************************************************************************
// Change this value for higher MCS values, i.e., 29 <= MCS <= 31
#define MAX_TBS_INDEX 31

int srslte_softbuffer_tx_init_scatter(srslte_softbuffer_tx_t *q, uint32_t nof_prb) {
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
  ...
  if(q != NULL) {
    ...
    ret = srslte_ra_tbs_from_idx_scatter(MAX_TBS_INDEX, nof_prb);
    if(ret != SRSLTE_ERROR) {
      q->max_cb = (uint32_t) ret / (SRSLTE_TCOD_MAX_LEN_CB - 24) + 1;
...
void srslte_softbuffer_tx_free_scatter(srslte_softbuffer_tx_t *q) {
  if(q) {
    if(q->buffer_b) {
      for(uint32_t i = 0; i < q->max_cb; i++) {
        if(q->buffer_b[i]) {
...
int srslte_softbuffer_rx_init_scatter(srslte_softbuffer_rx_t *q, uint32_t nof_prb) {
  int ret = SRSLTE_ERROR_INVALID_INPUTS;
...
clean_exit:
  if(ret != SRSLTE_SUCCESS) {
    srslte_softbuffer_rx_free_scatter(q);
  }

  return ret;
}

void srslte_softbuffer_rx_free_scatter(srslte_softbuffer_rx_t *q) {
  if(q) {
    if(q->buffer_f) {
      for(uint32_t i = 0; i < q->max_cb; i++) {
        if(q->buffer_f[i]) {
