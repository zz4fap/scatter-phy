...
bool disable_plots = true;
int cell_id = 0;
char *rf_args="";
float rf_gain=40.0;
float rf_freq=2400000000;
int nof_frames = -1;
uint32_t fft_size = 384;
float threshold = 4.0;
int N_id_2_sync = -1;
...
int main(int argc, char **argv) {
  ...
  float sfo = 0;
  int full_N_id_1 = -1000;
  srslte_cp_t cp1 = SRSLTE_CP_EXT;
  ...
  printf("Set RX rate: %.2f MHz\n", srslte_rf_set_rx_srate(&rf, srate, 0) / 1000000);
  ...
  while(frame_cnt < nof_frames || nof_frames == -1) {
    n = srslte_rf_recv(&rf, buffer, flen - peak_offset, 1, 0);
    if (n < 0) {
    ...
    if (peak_value >= threshold) {
      ...
      if (peak_idx >= fft_size) {
        ...
        if (sss_idx >= 0 && sss_idx < flen-fft_size) {
          full_N_id_1 = srslte_sss_synch_N_id_1(&sss, m0, m1);
          INFO("Full N_id_1: %d\n", full_N_id_1);
        ...
        if (peak_idx > 2*(fft_size + SRSLTE_CP_LEN_EXT(fft_size))) {
          cp1 = srslte_sync_detect_cp(&ssync, buffer, peak_idx);
          if (SRSLTE_CP_ISNORM(cp1)) {
            cp_is_norm++;
          }
        }

        if(peak_idx >= flen && full_N_id_1 == 0 && cp1 == SRSLTE_CP_NORM)
          printf("zz4fap: peak_pos: %d\n",peak_idx);
