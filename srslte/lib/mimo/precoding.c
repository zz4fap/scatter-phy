...
int srslte_predecoding_single(cf_t *y, cf_t *h, cf_t *x, int nof_symbols, float noise_estimate) {
  ...
  return srslte_predecoding_single_generic(y, h, x, NULL, nof_symbols, noise_estimate, nof_rxant, scaling);
}

/* ZF/MMSE SISO equalizer x=y(h'h+no)^(-1)h' (ZF if n0=0.0)*/
int srslte_predecoding_single_generic(cf_t *y, cf_t *h, cf_t *x, float *csi, int nof_symbols, float noise_estimate, uint32_t nof_rxant, float scaling) {
...
int srslte_predecoding_single_csi(cf_t *y, cf_t *h, cf_t *x, float *csi, int nof_rxant, int nof_symbols, float scaling, float noise_estimate) {
  int i = 0;
  ...
  for(; i < nof_symbols - SRSLTE_SIMD_CF_SIZE + 1; i += SRSLTE_SIMD_CF_SIZE) {
    ...
    for(int p = 0; p < nof_rxant; p++) {
      simd_cf_t _y = srslte_simd_cfi_load(&y[i]);
      simd_cf_t _h = srslte_simd_cfi_load(&h[i]);
  ...
  for(; i < nof_symbols; i++) {
    ...
    for(int p = 0; p < nof_rxant; p++) {
      r += y[i] * conj(h[i]);
      hh += (__real__ h[i] * __real__ h[i]) + (__imag__ h[i] * __imag__ h[i]);
    }
